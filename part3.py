...
@app.route('/ListG')  # 2.4
def page4():
    res = CentralRegistry().link_sentence_genes().sentences(request.args.get('gene'))
    return render_template('4_genesentencesresult.html', result = res)

@app.route('/RecD')  # 2.5
def page5():
    res = CentralRegistry().link_diseases()
    return render_template('5_disease.html', result = res)
...
@app.route('/ListD')  # 2.6
def page6():
    res = CentralRegistry().link_sentence_diseases().sentences(request.args.get('disease'))
