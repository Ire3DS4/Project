...
class CentralRegistry:  # Selects correct method based on user input
    def __init__(self):  # Data Readers & Registry
        ...
        self.__links = ['MD', 'Sem', 'RecG', 'ListG', 'RecD', 'ListD', 'Top10', 'GtoD', 'DtoG']
    ...
    def link_sentence_genes(self):  # 2.4 
        return GeneCovid(self.__data2).listSentences()

    def link_sentence_diseases(self):  # 2.6 
        return DiseaseCovid(self.__data1).listSentences()

    def link_top10(self):  # 2.7
        return Top10(self.__data2).record_association()
        
    def link_diseases_from_genes(self):  # 2.8
        return AssociatedDiseases(self.__data1, self.__data2).associate()

    def link_genes_from_diseases(self):  # 2.9
        return AssociatedGenes(self.__data1, self.__data2).associate()